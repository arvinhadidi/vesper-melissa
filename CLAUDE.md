...
- Background: The starry sky background is applied in two ways. For pages inside the `(app)` layout, `body::before` in globals.css handles it globally. For standalone routes outside `(app)` (e.g. `/onboarding`, `/personalisation`, `/signin`), add the background inline on the root div: `backgroundColor: '#1E1256', backgroundImage: 'linear-gradient(to bottom, rgba(30,18,86,0.6) 0%, rgba(30,18,86,0.45) 30%, rgba(30,18,86,0.8) 75%, rgba(30,18,86,1) 100%), url("/landing/starrysky.webp")', backgroundSize: 'auto, cover', backgroundPosition: 'center', backgroundAttachment: 'fixed'`. Do NOT use a plain solid background colour — always include the starry texture.
