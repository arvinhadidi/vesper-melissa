...
type ChatContext = DailyContext | SpreadContext;

function isSpreadContext(ctx: ChatContext): ctx is SpreadContext {
  return 'type' in ctx && ctx.type === 'spread';
}
...
export default function ChatPage() {
  ...
  useEffect(() => {
    const raw = sessionStorage.getItem(`chat-${chatId}`);
  ...
  const headerLabel = context
    ? isSpreadContext(context)
      ? context.questionText.length > 28
        ? context.questionText.slice(0, 28) + '…'
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh' }}>
      ...
      <div style={{
        ...
        <span style={{ fontFamily: 'var(--font-dm-serif-var), serif', fontSize: '16px', color: '#FAF7F0' }}>
          Melissa
        </span>

        <span style={{
          fontFamily: 'var(--font-dm-sans-var), sans-serif', fontSize: '11px', color: '#C9A84C',
